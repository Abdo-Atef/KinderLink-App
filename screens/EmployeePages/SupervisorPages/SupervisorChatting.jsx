...
export default function SupervisorChatting({ navigation }) {
  ...
  const handleSubmit = async (values) => {
    try {
      const docRef = doc(db, "chattingGroups", employeeData?.groups[0].id);
      const messsagesRef = collection(docRef, "messages");
      await addDoc(messsagesRef, {
        userName: employeeData?.name,
        userId: employeeData?.id,
        message: values.message,
        profileUrl: employeeData?.profilePicture.secure_url,
  ...
  useEffect(() => {
    if (employeeData?.groups.length > 0) {
      const docRef = doc(db, "chattingGroups", employeeData?.groups[0].id);
      const messsagesRef = collection(docRef, "messages");
  ...
  const renderEmployeeItem = ({ item }) => (
    ...
    >
      ...
      >
        ...
        <Text style={styles.timestamp}>{dayjs(item.createdAt.toDate()).format('HH:mm')}</Text>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      {employeeData?.groups.length > 0 ? (
        ...
        >
          <Text style={{ fontSize: 17, color: "blue" }}>You are not in a group</Text>
        </View>
      )}
    </SafeAreaView>
  );
