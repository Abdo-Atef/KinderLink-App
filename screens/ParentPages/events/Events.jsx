  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.textContainer}>
        ...
        <Text style={styles.description}>Details: {event.eventDescribtion}</Text>
  ...
  const getEvents = async () => {
    ...
    try {
      const { data } = await axios.get(
        `${BASE_URL}/childeren/events/getAllEventsToPaernt?state=availableForReverse`,
        { headers }
      );
      setDATA(data.events);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      const { data } = await axios.patch(
        `${BASE_URL}/childeren/events/reserveATicket/${EventId}`,
        {},
        { headers }
      );
      if (data.error) {
        Alert.alert("sorry you can't enroll ", data.error);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      {DATA?.length > 0 ? (
        ...
        >
          <Text style={{ color: "black", fontSize:wp(17) }}>There are no events yet</Text>
        </View>
      ) : (
        <ActivityIndicator
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalHeader}>
              <Text style={styles.modalHeaderText}>Enroll in Event</Text>
            </View>
            {/* Body */}
            <View style={styles.modalBody}>
              <Text style={styles.modalBodyText}>
                Are you sure you want to Enroll in the event or the trip?
              </Text>
            </View>
            {/* Footer */}
